	...
	public void Explode()
	{
        ...
        foreach (Collider2D en in players)
        {
            ...
            if (rb != null&&rb.tag == "Player")
            {
                ...
                // Find a vector from the bomb to the enemy.
                Vector3 deltaPos = rb.transform.position - transform.position;
                float value = (deltaPos.magnitude / (bombRadius*2));
               
                rb.gameObject.GetComponent<PlayerHealth>().Hurt((1-value)*25);

                // Apply a force in this direction with a magnitude of bombForce.
                Vector3 force = deltaPos.normalized * bombForce;
                rb.AddForce(force);
            }
            if(rb != null && rb.tag == "Player2")
            {
                // Find the Enemy script and set the enemy's health to zero.
                ...
                rb.gameObject.GetComponent<PlayerHealth2>().Hurt((1-value)*25);
